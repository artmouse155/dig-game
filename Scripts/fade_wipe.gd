...
func _ready():
	# Check if the current CanvasItem has a shader material
...
func fade_in():
	...
	tween.tween_method(set_shader_progress, 0.0, 1.0, ANIM_DURATION)
...
func fade_out():
	...
	tween.tween_method(set_shader_progress, 1.0, 0.0, ANIM_DURATION)
	tween.tween_callback(emit_completed_signal)
	await anim_completed
	hide()
